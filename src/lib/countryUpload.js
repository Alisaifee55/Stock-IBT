// =============================================================
// countryUpload.js — v2.1 — 12-09-2026
// Extracted from the old standalone AdminUpload.jsx panel so the
// upload control can live inline on each country's card instead of
// a separate full-width admin section. Country is now fixed per
// call site (one hook instance per card) instead of a dropdown.
//
// All upload/mapping/parsing logic is unchanged from v2.0 — only the
// "which country" selection moved from an in-panel <select> to
// "whichever card this hook instance is mounted on".
// =============================================================

import { useCallback, useEffect, useRef, useState } from 'react';
import { readXlsxStreaming } from './xlsxStreamReader';
import { supabase } from './supabaseClient';
import { runRefreshAndWait, withTimeout, SLOW_AFTER_MS } from './summaryRefresh';
...
// 2-digit (13 through 26 observed). Do not "fix" this to D/M.
...
/**
 * One upload pipeline bound to a single, fixed country. Mount one
 * instance per country card. status: idle | reading | uploading |
 * refreshing | done | error.
 */
export function useCountryUpload({ country, onUploaded }) {
  ...
  const handleFile = useCallback(
    async (file) => {
      ...
      try {
        ...
        if (totalMapped === 0) {
          throw new Error(
            `No rows were stored out of ${totalRead} read — ${dead.toLocaleString()} had no stock/sales/purchases and ${skipped.toLocaleString()} had an unrecognized shop code. Check its shops exist in Manage Shops (Shops tab).`
          );
        }
